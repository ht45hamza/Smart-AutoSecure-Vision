...
const DashboardContent = ({ onNavigate }) => {
    ...
    return (
        <div className="d-flex flex-column h-100 bg-dark-theme text-light font-sans animate-fade-in">
            {/* Header Removed (Moved to Global Shell) */}
            ...
            <div className="container-fluid p-4 flex-grow-1 overflow-hidden">
                <div className="row g-4 h-lg-100 overflow-y-auto overflow-lg-hidden pb-5 pb-lg-0">
                    {/* LEFT SIDEBAR: STATS (Grid on Mobile, Stacked on Desktop) */}
                    <div className="col-12 col-md-12 col-lg-3 col-xl-2 order-2 order-lg-1 h-auto h-lg-100 overflow-y-auto custom-scrollbar">
                        <div className="row g-3 flex-lg-column h-100">
                            ...
                            <div className="col-6 col-md-3 col-lg-12">
                                <div className="stat-card-enhanced border-start border-success border-4 shadow-sm h-100 d-flex flex-column justify-content-between" style={{ minHeight: '130px' }}>
                                    <div>
                            ...
                            <div className="col-6 col-md-3 col-lg-12">
                                <div className="stat-card-enhanced border-start border-warning border-4 shadow-sm h-100 d-flex flex-column justify-content-between" style={{ minHeight: '130px' }}>
                                    <div>
                            ...
                            {/* Suspects - Red */}
                            <div className="col-6 col-md-3 col-lg-12">
                                <div className="stat-card-enhanced border-start border-danger border-4 shadow-lg bg-danger-subtle h-100 d-flex flex-column justify-content-between" style={{ minHeight: '130px' }}>
                                    <div>
                            ...
                            <div className="col-6 col-md-3 col-lg-12 flex-lg-grow-1">
                                <div className="stat-card-enhanced border-start border-primary border-4 shadow-sm h-100 d-flex flex-column justify-content-center position-relative" style={{ minHeight: '130px' }}>
                    ...
                    <div className="col-12 col-md-12 col-lg-6 col-xl-7 d-flex flex-column gap-3 order-1 order-lg-2" style={{ minHeight: '400px' }}>
                        {/* Main Feed */}
                        <div className="main-video-container flex-grow-1 position-relative bg-black rounded-3 border border-dark-subtle overflow-hidden shadow-2xl d-flex align-items-center justify-content-center" style={{ minHeight: '300px' }}>
                            <div className="ratio ratio-16x9 w-100 h-100" style={{ maxHeight: '100%' }}>
                                {mainCameraId !== null ? (
                                    <>
                                        ...
                                        />
                                        {/* Overlays */}
                                        <div className="position-absolute top-0 start-0 m-3 px-3 py-1 bg-danger text-white rounded-pill small fw-bold shadow-sm animate-pulse z-10" style={{ height: 'fit-content', width: 'fit-content' }}>
                                            <span className="blink-dot bg-white me-2"></span>LIVE FEED
                                        </div>
                                        <button
                                            className="btn btn-dark btn-sm position-absolute top-0 end-0 m-3 border-secondary z-10"
                        ...
                        <div className="camera-grid-row d-flex gap-3 overflow-x-auto pb-2 custom-scrollbar" style={{ height: '120px' }}>
                            {cameras.map(cam => (
                                <div
                                    key={cam.id}
                                    className={`camera-thumbnail position-relative rounded-3 overflow-hidden flex-shrink-0 cursor-pointer ${mainCameraId === cam.id ? 'active-cam ring-2 ring-primary' : ''}`}
                    ...
                    <div className="col-12 col-md-12 col-lg-3 col-xl-3 d-flex flex-column gap-3 h-auto h-lg-100 order-3">
                        <button className="btn btn-primary w-100 py-3 fw-bold text-uppercase shadow-primary hover-scale transition-all" onClick={handleOpenAddModal}>
                            <i className="fas fa-plus-circle me-2"></i> Add Camera
                        </button>
                        {/* Admin Panel button removed */}

                        <div className="recent-activity-panel bg-activity rounded-3 border border-dark-subtle d-flex flex-column flex-grow-1 overflow-hidden mt-2 shadow-sm" style={{ height: '400px', maxHeight: '100%' }}>
                            ...
                            <div className="flex-grow-1 overflow-y-auto p-2 custom-scrollbar">
                                {stats.history && stats.history.length > 0 ? (
                                    stats.history.slice().reverse().map((item, idx) => (
                                        <div key={idx} className={`activity-item p-2 mb-2 rounded-3 d-flex align-items-center gap-3 border ${item.relation?.includes('Suspect') ? 'border-danger bg-danger-subtle' : 'border-dark-subtle bg-dark-item'} animate-fade-in`}>
                                            ...
                                            <div className="flex-grow-1 min-w-0">
                                                <div className="d-flex justify-content-between">
                                                    <span className="fw-bold text-sm text-light text-truncate">{item.name}</span>
            ...
            <div className="flex-grow-1 d-flex flex-column overflow-hidden position-relative w-100">
                ...
                <div className="d-flex justify-content-between align-items-center px-4 py-3 border-bottom border-dark-subtle bg-header z-10">
                    <div className="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <h5 className="fw-bold mb-0 text-uppercase tracking-wider d-none d-md-block text-light">Smart AutoSecure Vision</h5>
                            <div className="d-flex align-items-center gap-2 text-secondary" style={{ fontSize: '0.75rem' }}>
                                <span className="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 px-2 py-0.5 animate-pulse">LIVE DASHBOARD</span>
                            </div>
                        </div>
                    </div>
                    <div className="d-flex align-items-center gap-4 text-sm text-secondary">
                        <div className="d-none d-md-flex gap-3 align-items-center">
                            ...
                            <button className="btn btn-link p-0 text-secondary text-decoration-none hover-text-primary" onClick={() => setActiveView('about')}><i className="fas fa-info-circle me-1"></i> About</button>
                            <button className="btn btn-link p-0 text-secondary text-decoration-none hover-text-primary" onClick={() => setActiveView('help')}><i className="fas fa-question-circle me-1"></i> Help</button>
                            <button className="btn btn-link p-0 text-secondary text-decoration-none hover-text-primary" onClick={() => window.showComponent ? window.showComponent('login') : window.location.href = '/login'}><i className="fas fa-sign-in-alt me-1"></i> Login</button>
                        </div>
                        <div className="border-start border-dark-subtle ps-3 text-light fw-bold font-monospace">
